...
public partial class SettingsEditor : System.Windows.Controls.UserControl, IDisposable
{
    ...
    private readonly Func<AppSettings> _latest;
    ...
    public SettingsEditor(AppSettings current, Func<AppSettings, string?> apply, Func<RelayState?>? remoteState = null, Func<AppSettings>? latest = null, DesktopLayout? layout = null)
    {
        ...
        _latest = latest ?? (() => _current);
        _layout = layout ?? DesktopLayout.Load();
        ...
        FolderBox.Text = current.FileFolder;
        FileAccessBox.IsChecked = current.FileAccessEnabled;
        PushBox.IsChecked = current.FilePushEnabled;
        ClassicQr.IsChecked = !_layout.LightQr;
    ...
    private void UpdatePairing()
    {
        ...
        if (!_current.RemoteEnabled || host != _current.RelayHost || _pairing?.RoomId != invite!.Key.RoomId || _committed != invite.Key.RoomId + "@" + host)
        {
            PairingCode.Text = null; RoomText.Text = ""; CopyLinkButton.IsEnabled = false;
            NewKeyButton.IsEnabled = true; PairingHint.Text = Strings.T("D_SaveToPair"); return;
        }
    ...
    {
        _pairing = PairingKey.Create();
        if (TryInvite(RelayBox.Text.Trim(), out var invite)) Commit(invite!);
        UpdatePairing();
    }
    ...
    private void OnSaveClicked(object sender, RoutedEventArgs e)
    {
        ...
        settings.BridgeEnabled = BridgeBox.IsChecked == true;
        settings.BlenderPath = BlenderPathBox.Text.Trim();
        settings.HeadlessRenderEnabled = RenderBox.IsChecked == true;
        settings.FileFolder = FolderBox.Text.Trim();
        settings.FileAccessEnabled = FileAccessBox.IsChecked == true;
        settings.FilePushEnabled = PushBox.IsChecked == true;
        if (settings.HeadlessRenderEnabled && (!settings.RemoteEnabled || !File.Exists(settings.BlenderPath)))
        { ShowStatus(Strings.T("D_BlenderRequired")); return; }
        if ((settings.FileAccessEnabled || settings.FilePushEnabled) && (!settings.RemoteEnabled || !Directory.Exists(settings.FileFolder)))
        { ShowStatus(Strings.T("D_FolderRequired")); return; }
        if (settings.FilePushEnabled && !settings.FileAccessEnabled)
        { ShowStatus(Strings.T("D_ReadRequired")); return; }
        ...
        _committed = _pairing is null ? null : _pairing.RoomId + "@" + settings.RelayHost;
        UpdatePairing(); ShowStatus(Strings.T("D_Saved")); Saved?.Invoke();
    }
    ...
    {
        var dialog = new Microsoft.Win32.OpenFileDialog { Filter = "Blender|blender.exe" };
        if (dialog.ShowDialog(Window.GetWindow(this)) == true) BlenderPathBox.Text = dialog.FileName;
    }
