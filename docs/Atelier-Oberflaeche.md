...
| | Symptom | Where it is |
|---|---|---|
| 1 | ~~Cut-out PNGs show pixelated fog in the transparent areas~~ | **fixed** — an unplaced image layer ignored its own alpha channel |
| 2 | Dragging in the picture lags far behind the mouse | the drag goes through the path that reloads passes instead of the coarse recompute |
| 3 | The placement frame sits in the wrong place | the frame is mapped to the adorner's own size; that is only the picture's size in some layouts |
| 4 | Dragging affects the wrong layer, or none | no mode: whether the drag applies depends on invisible state |
| 5 | Reopening shows no images until one is added | the recipe is restored but the view is not rebuilt until something changes |
| 6 | A newly added layer shows as hidden, and the eye toggles the wrong way | the visibility of a newly added layer is not pushed into the row |
| 7 | Some PNGs do not appear at all | to be reproduced against the new test files — palette, greyscale+alpha, 16-bit and interlaced each take a different route through the decoder |
